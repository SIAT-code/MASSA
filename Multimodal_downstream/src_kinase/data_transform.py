...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = torch.tensor(df['labels'], dtype=torch.long)
...
def data_read(tokens_input, dataset_type, max_pro_seq_len, seed):
    file_path = os.path.join(tokens_input, "sequence_go_%s.txt" % dataset_type)
    df_tokens = pd.read_csv(file_path, sep=',')
    df_tokens.columns = ['molecule', 'uniprot_id', 'seq', 'go', 'label']

    seq_tokens, go_tokens, compound_tokens, labels = df_tokens['seq'].fillna('').values.tolist(), df_tokens['go'].fillna('').values.tolist(), df_tokens['molecule'].fillna('').values.tolist(), df_tokens['label'].fillna('').values.tolist()
