...
class ProLigandCrossFusion(nn.Module):
    def __init__(self, pro_hid_dim, compound_hid_dim, pro_n_heads, compound_n_heads, pro_n_layers, compound_n_layers, pro_pf_dim, compound_pf_dim, pro_dropout, compound_dropout):
        super().__init__()
        self.pro_layers = nn.ModuleList(
            [TransformerDecoderLayer(pro_hid_dim, pro_n_heads, pro_pf_dim, pro_dropout)
             for _ in range(pro_n_layers)])

        self.compound_layers = nn.ModuleList(
            [TransformerDecoderLayer(compound_hid_dim, compound_n_heads, compound_pf_dim, compound_dropout)
             for _ in range(compound_n_layers)])

        self.pro2ligand = nn.Linear(pro_hid_dim, compound_hid_dim)
        self.ligand2pro = nn.Linear(compound_hid_dim, pro_hid_dim)

    def forward(self, pro_feat, compound_feat, pro_mask=None, compound_mask=None):
        com_feat_for_pro = self.ligand2pro(compound_feat)
        for layer in self.pro_layers:
            pro_feat_new = layer(pro_feat, com_feat_for_pro, pro_mask, compound_mask)

        pro_feat_for_com = self.pro2ligand(pro_feat)
        for layer in self.compound_layers:
            compound_feat_new = layer(compound_feat, pro_feat_for_com, compound_mask, pro_mask)
...
class RegressionLayer(nn.Module):
    def __init__(self, pro_hid_dim):
        super().__init__()
        self.fc1 = nn.Linear(pro_hid_dim + 128, int((pro_hid_dim + 128)/2))
        self.fc2 = nn.Linear(int((pro_hid_dim + 128)/2), 1)

    def forward(self, pro_feat, mol_emb, pro_mask):
        ...
        con_emb = torch.cat((pro_feat, mol_emb), dim=1)
        con_emb = F.relu(self.fc1(con_emb))
        label = self.fc2(con_emb)
        label = label.squeeze(1)
        
        return label

class Predictor(nn.Module):
    def __init__(self, seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion, compound_encoder, regression_layer, add_structure, add_goterm):
        super().__init__()
        ...
        self.pro_emb_linear = nn.Linear(512*2, 512)
        self.regression_layer = regression_layer
        self.add_structure = add_structure
        self.add_goterm = add_goterm

    def make_masks(self, proteins_num, goterms_num, compounds_num, protein_max_len, goterm_max_len, compound_max_len):
        N = len(proteins_num)  # batch size
        # protein_max_len, goterm_max_len = torch.max(proteins_num), torch.max(goterms_num)
        ...
        for i in range(N):
            ...
            compound_mask[i, :compounds_num[i]] = 1
        ...
        protein_mask, goterm_mask, compound_mask = protein_mask.to(proteins_num.device), goterm_mask.to(goterms_num.device), compound_mask.to(compounds_num.device)
        return protein_mask, goterm_mask, compound_mask
    ...
    def forward(self, seq_feat, struc_feat, proteins_num, goterms, goterms_num, compounds_num, atoms, bonds, atom_degrees, bond_degrees, atom_masks):
        device = seq_feat.device
        proteins_mask, goterms_mask, compounds_mask = self.make_masks(proteins_num, goterms_num, compounds_num, seq_feat.shape[1], goterms.shape[1], atoms.shape[1])
        ...
        pro_emb = torch.cat((seq_emb, pro_emb), dim=2)
        pro_emb = self.pro_emb_linear(pro_emb)
        mol_emb, compound_emb = self.compound_encoder(atoms, bonds, atom_degrees, bond_degrees, atom_masks)
       
        label = self.regression_layer(pro_emb, mol_emb, proteins_mask)

        return label

    def __call__(self, data):
        seq_feat, struc_feat, gpu_split, goterms, proteins_num, goterms_num, compounds_num, atoms, bonds, atom_degrees, bond_degrees, atom_masks = data
        device = seq_feat.device
        ...
        label = self.forward(seq_feat, struc_feat, proteins_num, goterms, goterms_num, compounds_num, atoms, bonds, atom_degrees, bond_degrees, atom_masks)
        return label
