...
class Trainer(object):
    ...
    def train(self, dataset_tuple, device, dir_input, data_type):
        ...
        for step, batch in enumerate(zip(dataloader, dataloaderstruc)):
            ...
            labels = labels[:, :max_protein_len_batch]
            ...
            predict_labels, labels = remove_no_loss_calculation(predict_labels, labels, cate_num=3)
...
class Tester(object):
    ...
    def test(self, dataset_tuple, device, dir_input, data_type):
        ...
        for step, batch in enumerate(zip(dataloader, dataloaderstruc)):
            ...
            # EFL loss 要求 预测的 requires_grad = True, 只有 remote homology 任务才有这两行代码
