...
#         self.mol_attend = nn.ModuleList([nn.Linear(fingerprint_dim, fingerprint_dim) for t in range(T)])
        ...
        self.atom_dense = nn.Sequential(nn.Linear(fingerprint_dim, fingerprint_dim), nn.ReLU(), \
                                    nn.Linear(fingerprint_dim, fingerprint_dim), nn.ReLU(), \
                                    nn.Linear(fingerprint_dim, fingerprint_dim))

        self.mol_dense = nn.Sequential(nn.Dropout(p_dropout), \
                                    nn.Linear(fingerprint_dim, fingerprint_dim), \
                                    nn.LayerNorm(fingerprint_dim, eps=1e-12))

        
    def forward(self, atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask):
        # atom_list: tensor(batch_size, max_atom_len, num_atom_features)
        # bond_list: tensor(batch_size, max_bond_len, num_bond_features)
        # atom_degree_list: tensor(batch_size, max_atom_len, degrees_len)
        # bond_degree_list: tensor(batch_size, max_atom_len, degrees_len)
        # atom_mask: tensor(batch_size, max_atom_len, 1)
        atom_mask = atom_mask.unsqueeze(2)
        batch_size,mol_length,num_atom_feat = atom_list.size()
        # atom_feature: tensor(batch_size, max_atom_len, fingerprint_dim)
        atom_feature = F.leaky_relu(self.atom_fc(atom_list))
        bond_neighbor = [bond_list[i][bond_degree_list[i]] for i in range(batch_size)]
        # bond_neighbor: tensor(batch_size, max_atom_len, degrees_len, num_bond_features), eg.(32,178,6,10)
        bond_neighbor = torch.stack(bond_neighbor, dim=0)
        atom_neighbor = [atom_list[i][atom_degree_list[i]] for i in range(batch_size)]
        # atom_neighbor: tensor:(batch_size, max_atom_len, degrees_len, num_atom_features), eg.(32,178,6,39)
        atom_neighbor = torch.stack(atom_neighbor, dim=0)
        # then concatenate them
        # neighbor_feature: tensor(batch_size, max_atom_len, degrees_len, num_atom_features+num_bond_features), eg.(32,178,6,49)
        neighbor_feature = torch.cat([atom_neighbor, bond_neighbor],dim=-1)
        # neighbor_feature: tensor(batch_size, max_atom_len, degrees_len, fingerprint_dim), eg.(32,178,6,150)
        neighbor_feature = F.leaky_relu(self.neighbor_fc(neighbor_feature))

        # generate mask to eliminate the influence of blank atoms
        # attend_mask: tensor(batch_size, max_atom_len, degrees_len)
        attend_mask = atom_degree_list.clone()
        attend_mask[attend_mask != mol_length-1] = 1
        attend_mask[attend_mask == mol_length-1] = 0
        # attend_mask: tensor(batch_size, max_atom_len, degrees_len, 1)
        attend_mask = attend_mask.type(torch.cuda.FloatTensor).unsqueeze(-1)

        # softmax_mask: tensor(batch_size, max_atom_len, degrees_len)
        ...
        # softmax_mask: tensor(batch_size, max_atom_len, degrees_len, 1)
        ...
        # atom_feature_expand: tensor(batch_size, max_atom_len, degrees_len, fingerprint_dim), eg.(32,178,6,150)
        atom_feature_expand = atom_feature.unsqueeze(-2).expand(batch_size, mol_length, max_neighbor_num, fingerprint_dim)
        # feature_align: tensor(batch_size, max_atom_len, degrees_len, fingerprint_dim*2), eg.(32,178,6,300)
        feature_align = torch.cat([atom_feature_expand, neighbor_feature],dim=-1)
        # align_score: tensor(batch_size, max_atom_len, degrees_len, 1), eg.(32,178,6,1)
        align_score = F.leaky_relu(self.align[0](self.dropout(feature_align)))
        align_score = align_score + softmax_mask
        # attention_weight: tensor(batch_size, max_atom_len, degrees_len, 1), eg.(32,178,6,1)
        attention_weight = F.softmax(align_score,-2)

        attention_weight = attention_weight * attend_mask
        # neighbor_feature_transform: tensor(batch_size, max_atom_len, degrees_len, fingerprint_dim), eg.(32,178,6,150)
        neighbor_feature_transform = self.attend[0](self.dropout(neighbor_feature))
        # context: tensor(batch_size, max_atom_len, fingerprint_dim), eg.(32,178,150)
        context = torch.sum(torch.mul(attention_weight,neighbor_feature_transform),-2)
        context = F.elu(context)
        # context_reshape: tensor(batch_size*max_atom_len, fingerprint_dim), eg.(5696,150)
        context_reshape = context.view(batch_size*mol_length, fingerprint_dim)
        # atom_feature_reshape: tensor(batch_size*max_atom_len, fingerprint_dim), eg.(5696,150)
        ...
        # atom_feature: tensor(batch_size, max_atom_len, fingerprint_dim), eg.(32,178,150)
        atom_feature = atom_feature_reshape.view(batch_size, mol_length, fingerprint_dim)
        
        #do nonlinearity
        # activated_features: tensor(batch_size, max_atom_len, fingerprint_dim), eg.(32,178,150)
        ...
        for d in range(self.radius-1):
            ...
            # do nonlinearity
            activated_features = F.relu(atom_feature)
        # mol_feature: tensor(batch_size, fingerprint_dim), eg.(32,150)
        mol_feature = torch.sum(activated_features * atom_mask, dim=-2)# B * feature, all atom sum = mol
        
        # do nonlinearity
        # activated_features_mol: tensor(batch_size, fingerprint_dim), eg.(32,150)
        activated_features_mol = F.relu(mol_feature)           
        # mol_softmax_mask: tensor(batch_size, max_atom_len, 1), eg.(32,178,1)
        ...
        atom_feature = self.atom_dense(atom_feature)
        mol_feature = self.mol_dense(mol_feature)

        return mol_feature, atom_feature