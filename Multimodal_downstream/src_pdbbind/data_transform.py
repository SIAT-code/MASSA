...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        compounds_num = torch.tensor(df['compounds_num'], dtype=torch.long)
        labels = torch.tensor(df['labels'], dtype=torch.float32)
        atoms = torch.tensor(df['atoms'], dtype=torch.float32)
        bonds = torch.tensor(df['bonds'], dtype=torch.float32)
        atom_degrees = torch.tensor(df['atom_degrees'], dtype=torch.long)
        bond_degrees = torch.tensor(df['bond_degrees'], dtype=torch.long)
        atom_masks = torch.tensor(df['atom_masks'], dtype=torch.float32)
        data_list = [protein_tokens_pad, go_tokens_pad, pro_ids, goterms, proteins_num, goterms_num, compounds_num, labels, atoms, bonds, atom_degrees, bond_degrees, atom_masks]
        return data_list
...
def pack(pro_tokens, go_tokens, compound_tokens, pro_ids, goterms, labels, dataset_path, dataset_type):
    ...
    for compound_token in compound_tokens:
        compounds_num.append(len(compound_token))
        if len(compound_token) >= compounds_len:
            compounds_len = len(compound_token)
    compounds_num = torch.tensor(compounds_num, dtype=torch.long)
    ...
    # atoms：array, (batch_size, max_atom_len, num_atom_features), float64
    # bonds: array, (batch_size, max_bond_len, num_bond_features), float64
    # atom_degrees: array, (batch_size, max_atom_len, degrees_len), float64
    # bond_degrees: array, (batch_size, max_bond_len, degrees_len), float64
    # atom_masks: array, array, (batch_size, max_atom_len), float64
    atoms, bonds, atom_degrees, bond_degrees, atom_masks = load_degree_smiles(compound_tokens, os.path.join(dataset_path, 'smiles_feature_dict_%s.pkl' % dataset_type))
    ...
    data_pack_dict["compounds_num"] = list(compounds_num.numpy())
    data_pack_dict["labels"] = labels
    data_pack_dict["atoms"] = list(atoms)
    data_pack_dict["bonds"] = list(bonds)
    data_pack_dict["atom_degrees"] = list(atom_degrees)
    data_pack_dict["bond_degrees"] = list(bond_degrees)
    data_pack_dict["atom_masks"] = list(atom_masks)
    data_pack_df = pd.DataFrame(data_pack_dict)
    return data_pack_df

def transform_data(dataset, max_pro_seq_len, dataset_path, dataset_type):
    seq_tokens, go_tokens, compound_tokens, pro_sequences, goterms, pro_structures, labels = [], [], [], [], [], [], []
    for data in dataset:
        seq_token, go_token, compound_token, pro_sequence, goterm, pro_structure, label = data
        seq_tokens.append(seq_token)
        go_tokens.append(go_token)
        compound_tokens.append(compound_token)
        pro_sequences.append(pro_sequence)
        goterms.append(goterm)
        pro_structures.append(pro_structure)
        labels.append(label)
    
    data_pack_df = pack(seq_tokens, go_tokens, compound_tokens, pro_sequences, goterms, labels, dataset_path, dataset_type)
...
def data_read(tokens_input, dataset_type, max_pro_seq_len, seed):
    file_path = os.path.join(tokens_input, "samples_seq_mole_go_%s.txt" % dataset_type)
    df_tokens = pd.read_csv(file_path, sep=',', header=None)
    df_tokens.columns = ['pro_id', 'seq', 'smiles', 'rdkit_smiles', 'label', 'dataset_type', 'go']

    seq_tokens, go_tokens, compound_tokens, labels = df_tokens['seq'].fillna('').values.tolist(), df_tokens['go'].fillna('').values.tolist(), df_tokens['rdkit_smiles'].fillna('').values.tolist(), df_tokens['label'].fillna('').values.tolist()

    _, problem_indexs = search_problem_smiles(compound_tokens, os.path.join(tokens_input, 'smiles_feature_dict_%s.pkl' % dataset_type))

    seq_tokens_tmp = []
    for index, seq_token in enumerate(seq_tokens):
        seq_tokens_tmp.append(seq_token[: max_pro_seq_len])
    seq_tokens = seq_tokens_tmp

    """Load preprocessed data."""
    pro_ids = load_tensor_for_pro_ids(os.path.join(tokens_input, 'seqs_id_%s.txt' % dataset_type), torch.FloatTensor)

    goterms = load_tensor(os.path.join(tokens_input, 'goterms_%s' % dataset_type), torch.FloatTensor)

    cath = CATHDataset(os.path.join(tokens_input, 'structure_%s.jsonl' % dataset_type))
    ...
    for i in reversed(range(len(seq_tokens))):
        if len(seq_tokens[i]) != len(cath.data[i]['seq']) and (len(seq_tokens[i]) < max_pro_seq_len or len(cath.data[i]['seq']) < max_pro_seq_len):
            ...
            del seq_tokens[i]
            del go_tokens[i]
            del compound_tokens[i]
            del pro_ids[i]
            del goterms[i]
            del cath.data[i]
            del labels[i]
        if i in problem_indexs:
            problem_samples_count += 1
            del seq_tokens[i]
            del go_tokens[i]
            del compound_tokens[i]
            del pro_ids[i]
    ...
    dataset = list(zip(seq_tokens, go_tokens, compound_tokens, pro_ids, goterms, cath.data, labels))
