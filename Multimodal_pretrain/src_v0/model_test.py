...
class ProGoCrossFusion(nn.Module):
    def __init__(self, pro_hid_dim, go_hid_dim, pro_cate, go_cate, motif_cate, domain_cate, region_cate, pro_n_heads, go_n_heads, pro_n_layers, go_n_layers, pro_pf_dim, go_pf_dim, pro_dropout, go_dropout):
        super().__init__()
        ...
        self.pro_tokens_predictor = nn.Linear(pro_hid_dim, pro_cate)
        self.go_tokens_predictor = nn.Linear(go_hid_dim, go_cate)
        self.motif_tokens_predictor = nn.Linear(pro_hid_dim, motif_cate)
        self.domain_tokens_predictor = nn.Linear(pro_hid_dim, domain_cate)
        self.region_tokens_predictor = nn.Linear(pro_hid_dim, region_cate)
    ...
    def forward(self, pro_feat, go_feat, pro_mask=None, go_mask=None):
        ...
        for layer in self.pro_layers:
            # pro_feat_new = layer(pro_feat, go_feat_for_pro, pro_mask, go_mask.permute(0, 1, 3, 2))
        ...
        for layer in self.go_layers:
            # go_feat_new = layer(go_feat, pro_feat_for_go, go_mask, pro_mask.permute(0, 1, 3, 2))
            go_feat_new = layer(go_feat, pro_feat_for_go, go_mask, pro_mask)

        pro_tokens_predicted = self.pro_tokens_predictor(pro_feat_new)
        go_tokens_predicted = self.go_tokens_predictor(go_feat_new)
        motif_tokens_predicted = self.motif_tokens_predictor(pro_feat_new)
        domain_tokens_predicted = self.domain_tokens_predictor(pro_feat_new)
        region_tokens_predicted = self.region_tokens_predictor(pro_feat_new)

        return pro_tokens_predicted, go_tokens_predicted, motif_tokens_predicted, domain_tokens_predicted, region_tokens_predicted

class Predictor(nn.Module):
    def __init__(self, seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion):
        super().__init__()
    ...
    def make_masks(self, proteins_num, goterms_num, protein_max_len, goterm_max_len):
        ...
        # protein_mask = protein_mask.unsqueeze(1).unsqueeze(3)
        # goterm_mask = goterm_mask.unsqueeze(1).unsqueeze(3)
    ...
    def forward(self, seq_feat, struc_feat, proteins_num, goterms, goterms_num):
        ...
        goterms_meb = self.goterm_encoder(goterms, goterms_mask)
        end_time_5 = time.time()
        # print("time 5: %g" % (end_time_5-start_time_5))

        start_time_6 = time.time()
        pro_logit, go_logit, motif_logit, domain_logit, region_logit = self.pro_go_cross_fusion(seq_struc_emb, goterms_meb, proteins_mask, goterms_mask)
        end_time_6 = time.time()
        # print("time 6: %g" % (end_time_6-start_time_6))

        return pro_logit, go_logit, motif_logit, domain_logit, region_logit
    ...
    def __call__(self, data):
        ...
        pro_logit, go_logit, motif_logit, domain_logit, region_logit = self.forward(seq_feat, struc_feat, proteins_num, goterms, goterms_num)
        return pro_logit, go_logit, motif_logit, domain_logit, region_logit


if __name__ == '__main__':
    # x = torch.randn(2, 1022, 768)
    # model = SeqEncoder(768, 512, 3, 7, 1022)
    # out = model(x)
    # print(out.shape)

    goterms = torch.randn(2, 1022, 64)
    mask = torch.randn(2, 1022)
    goterms_mask = torch.ones_like(mask)
    go_model = GotermEncoder(64, 128, 4, 3, 512)
    out = go_model(goterms, goterms_mask)
    print(out.shape)
