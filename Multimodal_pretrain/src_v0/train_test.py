...
class Trainer(object):
    def __init__(self, model, lr, weight_decay, batch_size, gradient_accumulation, pro_cate, go_cate, motif_cate, domain_cate, region_cate):
        self.model = model
        ...
        self.go_cate = go_cate
        self.motif_cate = motif_cate
        self.domain_cate = domain_cate
        self.region_cate = region_cate

    def train(self, dataset_tuple, device, dir_input, n_gpu):
        self.model.train()
        ...
        # dataloaderstruc = torch_geometric.loader.DataLoader(dataset_structure, num_workers=4, sampler=datasampler, batch_size=self.batch_size)
        # dataloaderstruc = torch_geometric.loader.DataListLoader(dataset_structure, num_workers=4, batch_sampler=gvp.data.BatchSampler(dataset_structure.node_counts, max_nodes=max_nodes), batch_size=self.batch_size)
        dataloaderstruc = torch_geometric.loader.DataListLoader(dataset_structure, num_workers=4, batch_size=self.batch_size)
        Loss = nn.CrossEntropyLoss()
        loss_total, loss_pro, loss_go, loss_motif, loss_domain, loss_region = 0, 0, 0, 0, 0, 0
        ...
        for step, batch in enumerate(zip(dataloader, dataloaderstruc)):
            ...
            seq_tokens, go_tokens, motif_tokens, domain_tokens, region_tokens, pro_ids, goterms, proteins_num, goterms_num = batch1
            max_protein_len_batch = torch.max(proteins_num)
            max_goterms_len_batch = torch.max(goterms_num)
            seq_tokens, go_tokens, motif_tokens, domain_tokens, region_tokens = seq_tokens[:, :max_protein_len_batch], go_tokens[:, :max_goterms_len_batch], motif_tokens[:, :max_protein_len_batch], domain_tokens[:, :max_protein_len_batch], region_tokens[:, :max_protein_len_batch]
            seq_tokens, go_tokens, motif_tokens, domain_tokens, region_tokens = seq_tokens.to(device), go_tokens.to(device), motif_tokens.to(device), domain_tokens.to(device), region_tokens.to(device)
            ...
            sequences_mask, structure_mask, pro_mask_labels = pro_mask_tokens(seq_tokens, pro_seqs, batch2, proteins_num)
            goterms_mask, go_mask_labels = goterm_mask_tokens(go_tokens, goterms, goterms_num)
            pro_correct_tokens_label, go_correct_tokens_label, motif_correct_tokens_label, domain_correct_tokens_label, region_correct_tokens_label = pro_mask_labels, go_mask_labels, motif_tokens, domain_tokens, region_tokens
            end_time_2 = time.time()
            # print("time 2: %g" % (end_time_2-start_time_2))

            start_time_3 = time.time()
            data_pack = (sequences_mask, structure_mask, gpu_split, goterms_mask, proteins_num, goterms_num)
            pro_predict_tokens_label, go_predict_tokens_label, motif_predict_tokens_label, domain_predict_tokens_label, region_predict_tokens_label = self.model(data_pack)
            end_time_3 = time.time()
            # print("time 3: %g" % (end_time_3-start_time_3))

            start_time_4 = time.time()
            pro_predict_tokens_label, pro_correct_tokens_label = remove_no_loss_calculation(pro_predict_tokens_label, pro_correct_tokens_label, self.pro_cate)
            go_predict_tokens_label, go_correct_tokens_label = remove_no_loss_calculation(go_predict_tokens_label, go_correct_tokens_label, self.go_cate)
            motif_predict_tokens_label, motif_correct_tokens_label = remove_no_loss_calculation(motif_predict_tokens_label, motif_correct_tokens_label, self.motif_cate)
            domain_predict_tokens_label, domain_correct_tokens_label = remove_no_loss_calculation(domain_predict_tokens_label, domain_correct_tokens_label, self.domain_cate)
            region_predict_tokens_label, region_correct_tokens_label = remove_no_loss_calculation(region_predict_tokens_label, region_correct_tokens_label, self.region_cate)
            end_time_4 = time.time()
            # print("time 4: %g" % (end_time_4-start_time_4))

            start_time_5 = time.time()
            if pro_predict_tokens_label.shape[0]!=0:
                pro_loss = Loss(pro_predict_tokens_label, pro_correct_tokens_label)
            else:
                pro_loss = torch.tensor(0).to(device)
            if go_predict_tokens_label.shape[0]!=0:
                go_loss = Loss(go_predict_tokens_label, go_correct_tokens_label)
            else:
                go_loss = torch.tensor(0).to(device)
            if motif_predict_tokens_label.shape[0]!=0:
                motif_loss = equalized_focal_loss(motif_predict_tokens_label, motif_correct_tokens_label)
            else:
                motif_loss = torch.tensor(0).to(device)
            if domain_predict_tokens_label.shape[0]!=0:
                domain_loss = equalized_focal_loss(domain_predict_tokens_label, domain_correct_tokens_label)
            else:
                domain_loss = torch.tensor(0).to(device)
            if region_predict_tokens_label.shape[0]!=0:
                region_loss = equalized_focal_loss(region_predict_tokens_label, region_correct_tokens_label)
            else:
                region_loss = torch.tensor(0).to(device)
            all_loss = pro_loss+go_loss+motif_loss+domain_loss+region_loss

            loss_total += all_loss.item()
            all_loss /= self.gradient_accumulation
            all_loss.backward()
            ...
            loss_go += go_loss.item()
            loss_motif += motif_loss.item()
            loss_domain += domain_loss.item()
            loss_region += region_loss.item()
        ...
        return loss_total/(step+1), loss_pro/(step+1), loss_go/(step+1), loss_motif/(step+1), loss_domain/(step+1), loss_region/(step+1)

class Tester(object):
    def __init__(self, model, batch_size, pro_cate, go_cate, motif_cate, domain_cate, region_cate):
        self.model = model
        self.batch_size = batch_size
        self.pro_cate = pro_cate
        self.go_cate = go_cate
    ...
    def test(self, dataset_tuple, device, dir_input, n_gpu):
        self.model.eval()
        dataset, dataset_structure = dataset_tuple
        datasampler = SequentialSampler(dataset)
        dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.batch_size)
        ...
        for step, batch in enumerate(zip(dataloader, dataloaderstruc)):
            ...
            # sequences_mask, structure_mask, pro_mask_labels = pro_mask_tokens(seq_tokens, pro_seqs, (h_V, h_E, edge_index, seq), proteins_num)
            sequences_mask, structure_mask, pro_mask_labels = pro_mask_tokens(seq_tokens, pro_seqs, batch2, proteins_num)
            goterms_mask, go_mask_labels = goterm_mask_tokens(go_tokens, goterms, goterms_num)
            pro_correct_tokens_label, go_correct_tokens_label, motif_correct_tokens_label, domain_correct_tokens_label, region_correct_tokens_label = pro_mask_labels, go_mask_labels, motif_tokens, domain_tokens, region_tokens
            ...
            if go_predict_tokens_label.shape[0]!=0:
                ...
                go_loss = torch.tensor(0).to(device)
            if motif_predict_tokens_label.shape[0]!=0:
                motif_loss = Loss(motif_predict_tokens_label, motif_correct_tokens_label)
            else:
                motif_loss = torch.tensor(0).to(device)
            if domain_predict_tokens_label.shape[0]!=0:
                domain_loss = Loss(domain_predict_tokens_label, domain_correct_tokens_label)
            else:
                domain_loss = torch.tensor(0).to(device)
            if region_predict_tokens_label.shape[0]!=0:
                region_loss = Loss(region_predict_tokens_label, region_correct_tokens_label)
            else:
                region_loss = torch.tensor(0).to(device)
