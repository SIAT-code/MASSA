...
class GNN_DATA:
    ...
    def get_feature_pretrained(self, pseq_path, pre_emb_path):
        ...
        for name in tqdm(self.protein_name.keys()):
            self.protein_dict[name] = np.array(self.pretrained_emb_dict[name.split('.')[1]])  # pretrained_emb_dict[name]: (512, )
    ...
    def get_feature_origin(self, pseq_path, vec_path):
        ...
        self.protein_dict = {}
        for name in tqdm(self.protein_name.keys()):
            self.protein_dict[name] = self.pvec_dict[name]  # pvec_dict[name]: (2000, 13)
