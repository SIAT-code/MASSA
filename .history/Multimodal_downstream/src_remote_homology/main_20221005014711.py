...
if __name__ == "__main__":
    ...
    # 数据文件路径
    data_scale = "12312"  # 100, 12312
    dir_input = "dataset/pretrain_benchmark/remote_homology/sequence_go_structure_%s/" % data_scale
    train_tokens_input = "dataset/pretrain_benchmark/remote_homology/sequence_go_structure_%s/sequence_go_train.txt" % data_scale
    dev_tokens_input = "dataset/pretrain_benchmark/remote_homology/sequence_go_structure_%s/sequence_go_valid.txt" % data_scale
    test_tokens_input = "dataset/pretrain_benchmark/remote_homology/sequence_go_structure_%s/sequence_go_test.txt" % data_scale
    ...
    if do_train:
        ...
        path_model = 'src_remote_homology/output/model-%s/' % current_time
        """Output files."""
        ...
        plot_train_dev_metric(list(range(1, epochs+1)), loss_train_epochs, loss_dev_epochs, path_model, 'loss', 'remote_homology')
        plot_train_dev_metric(list(range(1, epochs+1)), acc_train_epochs, acc_dev_epochs, path_model, 'ACC', 'remote_homology')
    ...
    if do_test:
        if not best_model:
            best_model_state_dict = torch.load("output/model-2022-06-01-16:58:21/model--2022-06-01-16:58:21.pth", map_location=torch.device('cpu'))
