...
class Trainer(object):
    ...
    def train(self, dataset_tuple, device, dir_input, data_type):
        self.model.train()
        dataset, dataset_structure_1, dataset_structure_2 = dataset_tuple
        datasampler = SequentialSampler(dataset)
        dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.batch_size)
        dataloaderstruc_1 = torch_geometric.loader.DataListLoader(dataset_structure_1, num_workers=4, batch_size=self.batch_size)
        dataloaderstruc_2 = torch_geometric.loader.DataListLoader(dataset_structure_2, num_workers=4, batch_size=self.batch_size)
        Loss = nn.MSELoss()
        loss_total = 0
        ...
        for step, batch in enumerate(zip(dataloader, dataloaderstruc_1, dataloaderstruc_2)):
            start_time_batch = time.time()
            start_time_1 = time.time()
            batch1, batch2, batch3 = batch
            gpu_split = torch.tensor(list(range(batch1[0].shape[0]))).to(device)
            seq_tokens_1, go_tokens_1, pro_ids_1, goterms_1, proteins_num_1, goterms_num_1, labels, seq_tokens_2, go_tokens_2, pro_ids_2, goterms_2, proteins_num_2, goterms_num_2 = batch1

            max_protein_len_batch = torch.max(proteins_num_1)
            max_goterms_len_batch = torch.max(goterms_num_1)
            seq_tokens_1, go_tokens_1 = seq_tokens_1[:, :max_protein_len_batch], go_tokens_1[:, :max_goterms_len_batch]
            seq_tokens_1, go_tokens_1, labels = seq_tokens_1.to(device), go_tokens_1.to(device), labels.to(device)

            all_seqs_emb = []
            for pro_id in pro_ids_1:
                ...
                if one_seq_emb.shape[0] > max_protein_len_batch:
                    all_seqs_emb.append(proteins_new.tolist())
                    continue
                proteins_new[:one_seq_emb.shape[0], :] = one_seq_emb   
                all_seqs_emb.append(proteins_new.tolist())
            pro_seqs_1 = torch.tensor(all_seqs_emb, dtype=torch.float32)
            pro_seqs_1 = pro_seqs_1.to(device)

            pro_seqs_1, goterms_1 = pro_seqs_1[:, :max_protein_len_batch, :], goterms_1[:, :max_goterms_len_batch, :]
            pro_seqs_1, goterms_1, proteins_num_1, goterms_num_1 = \
                pro_seqs_1.to(device), goterms_1.to(device), proteins_num_1.to(device), goterms_num_1.to(device)

            max_protein_len_batch = torch.max(proteins_num_2)
            max_goterms_len_batch = torch.max(goterms_num_2)
            seq_tokens_2, go_tokens_2 = seq_tokens_2[:, :max_protein_len_batch], go_tokens_2[:, :max_goterms_len_batch]
            seq_tokens_2, go_tokens_2 = seq_tokens_2.to(device), go_tokens_2.to(device)

            all_seqs_emb = []
            for pro_id in pro_ids_2:
                seq_emb_path = os.path.join(os.path.join(dir_input, 'sequences_emb_%s' % data_type), pro_id+".npy")
                one_seq_emb = np.load(seq_emb_path, allow_pickle=True)
                proteins_new = np.zeros((max_protein_len_batch, one_seq_emb.shape[1]))
                proteins_new[:one_seq_emb.shape[0], :] = one_seq_emb   
                all_seqs_emb.append(proteins_new.tolist())
            pro_seqs_2 = torch.tensor(all_seqs_emb, dtype=torch.float32)
            pro_seqs_2 = pro_seqs_2.to(device)

            pro_seqs_2, goterms_2 = pro_seqs_2[:, :max_protein_len_batch, :], goterms_2[:, :max_goterms_len_batch, :]
            pro_seqs_2, goterms_2, proteins_num_2, goterms_num_2 = \
                pro_seqs_2.to(device), goterms_2.to(device), proteins_num_2.to(device), goterms_num_2.to(device)

            batch2 = [batch_each_sample.to(device) for batch_each_sample in batch2]
            batch3 = [batch_each_sample.to(device) for batch_each_sample in batch3]
            end_time_1 = time.time()
            # print("time 1: %g" % (end_time_1-start_time_1))

            start_time_3 = time.time()
            data_pack = (pro_seqs_1, batch2, proteins_num_1, goterms_1, goterms_num_1, pro_seqs_2, batch3, proteins_num_2, goterms_2, goterms_num_2, gpu_split)
            ...
            all_predict_labels += predict_labels.tolist()
