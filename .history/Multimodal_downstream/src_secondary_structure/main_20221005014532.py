...
def data_read(tokens_input, data_type):
    ...
    seq_tokens_tmp, labels_tmp = [], []
    for seq_token, label in zip(seq_tokens, labels):
        seq_tokens_tmp.append(seq_token[: max_pro_seq_len])
        labels_tmp.append(label[: max_pro_seq_len])
...
if __name__ == "__main__":
    ...
    # 数据文件路径
    data_scale = "100"  # 100, 8678
    dir_input = "dataset/pretrain_benchmark/secondary_structure/sequence_go_structure_%s/" % data_scale
    train_tokens_input = "dataset/pretrain_benchmark/secondary_structure/sequence_go_structure_%s/sequence_go_train.txt" % data_scale
    dev_tokens_input = "dataset/pretrain_benchmark/secondary_structure/sequence_go_structure_%s/sequence_go_valid.txt" % data_scale
    test_tokens_input = "dataset/pretrain_benchmark/secondary_structure/sequence_go_structure_%s/sequence_go_test.txt" % data_scale
    ...
    model = Predictor(seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion, classification_layer, add_structure, add_goterm)
    ...
    if do_train:
        ...
        path_model = 'src_secondary_structure/output/model-%s/' % current_time
        """Output files."""
        ...
        results = ('Epoch\tTime\tLoss_train\tLoss_dev\tacc_train\tacc_dev')
        ...
        for epoch in range(1, epochs+1):
            ...
            acc_train_epoch = "%.3f" % acc_train; acc_dev_epoch = "%.3f" % acc_dev
            results = [epoch, spend_time_epoch, loss_train_epoch, loss_dev_epoch, acc_train_epoch, acc_dev_epoch]
            ...
            if loss_dev < min_loss_dev:
                ...
                acc_test = accuracy_score(all_real_labels_test, all_predict_labels_test)
                with open(file_test_results, 'a') as f_test_results:
                    f_test_results.write("The best model is in epoch %d. Test results: Loss: %.3f, ACC: %.3f\n" % (epoch, loss_test, acc_test))
        ...
        dict_loss['acc_train_all'] = acc_train_epochs
        dict_loss['acc_dev_all'] = acc_dev_epochs

        df_loss = pd.DataFrame(dict_loss)
        df_loss.to_csv(file_loss)

        plot_train_dev_metric(list(range(1, epochs+1)), loss_train_epochs, loss_dev_epochs, path_model, 'loss', 'secondary_structure')
        plot_train_dev_metric(list(range(1, epochs+1)), acc_train_epochs, acc_dev_epochs, path_model, 'ACC', 'secondary_structure')
    ...
    if do_test:
        if not best_model:
            best_model_state_dict = torch.load("output/model-2022-06-01-16:58:21/model--2022-06-01-16:58:21.pth")
        ...
        print("Test results: Loss: %.3f, ACC: %.3f" % (loss_test, acc_test))
