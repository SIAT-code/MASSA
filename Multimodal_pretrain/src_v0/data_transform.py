...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        motif_tokens_pad = torch.tensor(df['motif_tokens_pad'], dtype=torch.long)
        domain_tokens_pad = torch.tensor(df['domain_tokens_pad'], dtype=torch.long)
        region_tokens_pad = torch.tensor(df['region_tokens_pad'], dtype=torch.long)
        pro_ids = df['pro_ids']
        ...
        data_list = [protein_tokens_pad, go_tokens_pad, motif_tokens_pad, domain_tokens_pad, region_tokens_pad, pro_ids, goterms, proteins_num, goterms_num]
        return data_list
...
def pack(pro_tokens, go_tokens, pro_ids, goterms, motif_tokens, domain_tokens, region_tokens, dataset_path):
    ...
    motif2id_path = os.path.join(dataset_path, "motif2id.json")
    with open(motif2id_path) as f:
        motif2id_dict = json.load(f)
    domain2id_path = os.path.join(dataset_path, "domain2id.json")
    with open(domain2id_path) as f:
        domain2id_dict = json.load(f)
    region2id_path = os.path.join(dataset_path, "region2id.json")
    with open(region2id_path) as f:
        region2id_dict = json.load(f)
    ...
    motif_tokens_pad = []
    for motif_token in motif_tokens:
        motif_tokens_pad.append([motif2id_dict[t] if t in motif2id_dict else -100 for t in motif_token] + [-100]*(proteins_len-len(motif_token)))
    motif_tokens_pad = torch.tensor(motif_tokens_pad, dtype=torch.long)

    domain_tokens_pad = []
    for domain_token in domain_tokens:
        domain_tokens_pad.append([domain2id_dict[t] if t in domain2id_dict else -100 for t in domain_token] + [-100]*(proteins_len-len(domain_token)))
    domain_tokens_pad = torch.tensor(domain_tokens_pad, dtype=torch.long)

    region_tokens_pad = []
    for region_token in region_tokens:
        region_tokens_pad.append([region2id_dict[t] if t in region2id_dict else -100 for t in region_token] + [-100]*(proteins_len-len(region_token)))
    region_tokens_pad = torch.tensor(region_tokens_pad, dtype=torch.long)
    ...
    data_pack_dict["motif_tokens_pad"] = list(motif_tokens_pad.numpy())
    data_pack_dict["domain_tokens_pad"] = list(domain_tokens_pad.numpy())
    data_pack_dict["region_tokens_pad"] = list(region_tokens_pad.numpy())
...
def transform_data(dataset, max_pro_seq_len, dataset_path):
    seq_tokens, go_tokens, pro_sequences, goterms, pro_structures, motif_tokens, domain_tokens, region_tokens = [], [], [], [], [], [], [], []
    for data in dataset:
        seq_token, go_token, pro_sequence, goterm, pro_structure, motif_token, domain_token, region_token = data
    ...
    data_pack_df = pack(seq_tokens, go_tokens, pro_sequences, goterms, motif_tokens, domain_tokens, region_tokens, dataset_path)
