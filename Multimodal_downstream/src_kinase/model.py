...
class ClassificationLayer(nn.Module):
    def __init__(self, pro_hid_dim):
        ...
        self.fc2 = nn.Linear(int(pro_hid_dim/2), 2)
...
class Predictor(nn.Module):
    def __init__(self, seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion, compound_encoder, pro_ligand_cross_fusion, classification_layer, add_structure, add_goterm):
        super().__init__()
        ...
        self.pro_ligand_cross_fusion = pro_ligand_cross_fusion
    ...
    def forward(self, seq_feat, struc_feat, proteins_num, goterms, goterms_num, compounds_num, atoms, bonds, atom_degrees, bond_degrees, atom_masks):
        ...
        _, compound_emb = self.compound_encoder(atoms, bonds, atom_degrees, bond_degrees, atom_masks)
        pro_emb = self.pro_ligand_cross_fusion(pro_emb, compound_emb, proteins_mask, compounds_mask)

        label = self.classification_layer(pro_emb, proteins_mask)
